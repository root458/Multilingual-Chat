...
import 'package:multilingual_chat/services/speak_service.dart';
...
class MessageBox extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Row(
        ...
        children:
            _determineOrientation(message, _languageCodeService.languageCode),
      ),
    );
  }
  ...
  List<Widget> _determineOrientation(Message message, List<String> code) {
    if (message.isMine) {
      return [
        ...
        Flexible(
          ...
          child: FutureBuilder(
            future: TranslationService.translateMessage(
                ...
                toLanguageCode: code[0]),
            initialData: message.messageText,
            builder: (context, AsyncSnapshot<String> translatedText) {
              return GestureDetector(
                onTap: () async {
                  // On tap action on a message, speak the message
                  if (message.messageText != translatedText.data) {
                    await SpeakService().speak(words: translatedText.data!, langej: code[1]);
                  }
                },
                child: Container(
                  child: Padding(
